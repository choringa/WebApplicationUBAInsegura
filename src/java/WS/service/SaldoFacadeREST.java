...
@Stateless
@Path("ws.saldo")
public class SaldoFacadeREST extends AbstractFacade<Saldo> {

    @PersistenceContext(unitName = "WebApplicationUBATestPU")
    private EntityManager em;

    public SaldoFacadeREST() {
        super(Saldo.class);
    }
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public String edit(@PathParam("id") Integer id, Saldo entity) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Saldo> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
